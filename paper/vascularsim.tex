...
% ============================================================
\begin{abstract}
Autonomous navigation of surgical microbots through vascular networks is a critical capability for targeted drug delivery, thrombectomy, and diagnostic procedures.
While reinforcement learning (RL) has shown promise for endovascular navigation, progress is hindered by the absence of an open-source simulation platform that integrates vascular anatomy, multi-physics modeling, and standardized RL interfaces.
We present \textsc{VascularSim}, a modular Python framework that provides: (1)~graph-based vascular anatomy representation with a TubeTK medical imaging data pipeline, (2)~three Gymnasium-compatible RL environments with progressively richer physics, (3)~analytical hemodynamic and magnetic field models, (4)~a neural flow surrogate for real-time prediction, and (5)~a benchmark suite across five difficulty tiers.
In experiments with Proximal Policy Optimization (PPO), trained agents achieve 99\% navigation success with optimal path efficiency (ratio~1.00), matching the shortest-path oracle and outperforming random baselines (52\%), after 200k training steps completed in 67 seconds on a single CPU.
...
\paragraph{Hemodynamic simulation.}
...
Both are designed for offline high-fidelity computation and cannot provide the real-time performance (${<}$1\,ms per step) required for RL training.
...
\textsc{VascularSim} is organized into five modules: data ingestion, graph representation, RL environments, physics models, and benchmarking.
...
\begin{tikzpicture}[
    ...
    block/.style={draw, rounded corners, fill=blue!8, minimum width=2.0cm, minimum height=0.5cm, align=center},
    env/.style={draw, rounded corners, fill=green!10, minimum height=0.45cm, align=center, inner sep=3pt},
    phys/.style={draw, rounded corners, fill=orange!12, minimum height=0.45cm, align=center, inner sep=3pt},
    side/.style={draw, rounded corners, fill=yellow!15, minimum height=0.45cm, align=center, inner sep=3pt},
    arr/.style={-{Stealth[length=2mm]}, thick},
]
% Data pipeline
\node[block] (tre) at (0, 0) {TubeTK .tre files};
\node[block] (parser) at (0, -0.9) {\texttt{parse\_tre()}};
\node[block] (graph) at (0, -1.8) {\textbf{VascularGraph}\\NetworkX DiGraph};

% Environments (generously spaced row)
\node[env] (vnav) at (-2.2, -3.1) {VascularNav\\-v0};
\node[env] (fnav) at (0, -3.1) {FlowAware\\Nav-v0};
\node[env] (mnav) at (2.2, -3.1) {Magnetic\\Nav-v0};

% Physics models (right side, stacked)
\node[phys] (hemo) at (3.8, -2.6) {Hemo-\\dynamics};
\node[phys] (mag) at (3.8, -3.5) {Magnetic\\Fields};

% Training pipeline
\node[block, fill=red!8] (flat) at (0, -4.3) {\texttt{FlatNavObservation}};
\node[block, fill=red!8] (ppo) at (0, -5.2) {SB3 PPO Training};

% Benchmark suite (left side)
\node[side] (bench) at (-3.8, -3.1) {Benchmark\\5 Tiers};
...
% Physics integration (dashed arrows)
\draw[arr, dashed] (hemo.west) -- (fnav.north east);
\draw[arr, dashed] (mag.west) -- (mnav.east);
\draw[arr, dashed] (bench.east) -- (vnav.west);

\end{tikzpicture}%
}
\caption{VascularSim system architecture. Solid arrows show data flow from medical imaging to RL training. Dashed arrows show physics integration into specialized environments.}
...
The graph is constructed from \texttt{Tube} objects via the \texttt{from\_tubes()} factory method.
...
For RL navigation, an undirected view of the graph is constructed so that the agent can traverse edges in both directions, reflecting the physical bidirectionality of blood vessels.
...
Episodes are initialized with start--target pairs at least 5 graph hops apart when possible.
...
\paragraph{SB3 Compatibility.}
A \texttt{FlatNavObservation} wrapper flattens the dictionary observation into a $\text{Box}(8,)$ vector $[\mathbf{p}_\text{agent}, \mathbf{p}_\text{target}, \hat{n}, d]$ suitable for Stable-Baselines3's \texttt{MlpPolicy}.
...
Node pressures are distributed linearly from the inlet pressure $P_\text{in} = 13{,}332$\,Pa (100\,mmHg) to the outlet pressure $P_\text{out} = 2{,}666$\,Pa (20\,mmHg) based on topological distance from the graph root.
...
Wall shear stress is computed as $\tau_w = 4 \mu v / r$.
...
Each tier is procedurally generated with a fixed random seed for reproducibility.
...
Results are exported as JSON for reproducible comparison.
...
The PPO agent achieves 99\% success with a path length ratio of 1.00, indicating it follows the shortest graph path in nearly all episodes.
...
The current version uses discrete node-to-node navigation rather than continuous position control.
Vessel walls are rigid (no compliance or deformation).
The hemodynamic model does not capture pulsatile flow or non-Newtonian rheology.
The PPO results in Section~\ref{sec:experiments} are evaluated on the default 30-node environment; per-tier PPO training across all five benchmark tiers is left to future work.
These simplifications are deliberate for the v0.1 release and represent a starting point for the community to extend.
